...
class DataSet(tordata.Dataset):
    def __init__(self, data_cfg, training):
        ...
        if data_cfg['test_dataset_name'] == 'BarbieGait':
            self.label_list = [seq_info[0] for seq_info in self.seqs_info]
            self.types_list = [seq_info[1] for seq_info in self.seqs_info]
            self.p_types_list = [seq_info[1].split('-')[0] for seq_info in self.seqs_info]
            self.g_types_list = [seq_info[1].split('-')[0]+'-'+seq_info[1].split('-')[1] for seq_info in self.seqs_info]
            self.views_list = [seq_info[2] for seq_info in self.seqs_info]
            self.thk_list = [seq_info[3] for seq_info in self.seqs_info]
            self.clo_list = [seq_info[4] for seq_info in self.seqs_info]
            ...
            self.thk_set = sorted(list(set(self.thk_list)))
            self.clo_set = sorted(list(set(self.clo_list)))
            self.seqs_data = [None] * len(self)
            self.indices_dict = {label: [] for label in self.label_set}

            self.indices_view_dict = {views: [] for views in self.views_list}
            self.indices_thk_dict = {thk: [] for thk in self.thk_set}
            self.indices_clo_dict = {cloid: [] for cloid in self.clo_set}

            for i, seq_info in enumerate(self.seqs_info):
                self.indices_dict[seq_info[0]].append(i)

            for i, seq_info in enumerate(self.seqs_info):
                self.indices_view_dict[seq_info[2]].append(i)

            for i, seq_info in enumerate(self.seqs_info):
                self.indices_thk_dict[seq_info[3]].append(i)
            for i, seq_info in enumerate(self.seqs_info):
                self.indices_clo_dict[seq_info[4]].append(i)
    ...
    def __dataset_parser(self, data_config, training):
        ...
        def get_seqs_info_list(label_set,data_config):
            seqs_info_list = []
            for lab in label_set:
                # for thk in sorted(os.listdir(osp.join(dataset_root, lab))):
                    for typ in sorted(os.listdir(osp.join(dataset_root, lab))):
                        for vie in sorted(os.listdir(osp.join(dataset_root, lab, typ))):
                            if data_config['test_dataset_name'] == 'BarbieGait':
                                ...
                                if not training:
                                    cloid = typ.split('-')[2][5:]
                                    closeq_id = typ.split('-')[3]
                                    if thk!='thick0' and closeq_id != '00':
                                        continue
                                                             # if int(cloid) not in range(10):
                                #     continue
                                # if int(thkid) not in range(cloththk):
                                #     continue
                                ###
                                seq_info_new = [lab, typ, vie, thk, cloid]
                                ###
                                ...
                                if seq_dirs != []:
                                    ...
                                    seqs_info_list.append([*seq_info_new, seq_dirs])
        ...
        #     train_set,data_config, data_config['thknum_train'], data_config['clothnum_train']) if training else get_seqs_info_list(test_set,data_config, data_config['thknum_test'], data_config['clothnum_test'])
        self.seqs_info = get_seqs_info_list(
            train_set,data_config) if training else get_seqs_info_list(test_set,data_config)
