...
class ToFloat():
    ...
    def __call__(self, x):
        ...
        if not x.dtype.name.startswith("float"):
            x = x.astype(np.float32)
        return x

class ColorJitterCHW:
    ...
    def adjust_saturation(self, img, factor):
        img_gray = np.expand_dims(0.299 * img[0] + 0.587 * img[1] + 0.114 * img[2], axis=0)
        return np.clip((img - img_gray) * factor + img_gray, 0, 255)

    def adjust_hue(self, img, delta):
        # Convert to HLS using OpenCV, apply hue change, convert back
    ...
    def jitter(self, img):
        ...
        if self.hue > 0:
            ...
            img = self.adjust_hue(img, delta)

        return img

    def __call__(self, x):
        # x: numpy array of shape (N, C, H, W), dtype should be uint8 or float32 in [0, 255]
        assert isinstance(x, np.ndarray) and x.ndim == 4
        out = np.empty_like(x)
        for i in range(x.shape[0]):
            out[i] = self.jitter(x[i])
        return out
...
class RgbToGrayTransform():
    def __init__(self):
        # RGB转灰度的标准系数
        self.weights = np.array([0.2989, 0.5870, 0.1140]).reshape((1, 3, 1, 1))

    def __call__(self, x):
        """
        输入: x.shape = [N, 3, H, W]，RGB图像
        输出: gray.shape = [N, 1, H, W]，灰度图像
        """
        return (x * self.weights).sum(axis=1, keepdims=True)
