...
class CollateFn(object):
    def __init__(self, label_set, sample_config):
        ...
        if 'contournum' in sample_config.keys():
            self.contournum = True
        else:
            self.contournum = False
        # fixed cases
    ...
    def __call__(self, batch):
        ...
        def calculate_complexities(frames):
            complexities = []
            ...
            for frame in frames:
                ...
                if contours:
                    # 找出最长的轮廓
                    max_contour = max(contours, key=lambda contour: cv2.arcLength(contour, True))
                    # 计算多边形的复杂度
                    epsilon = 0.01 * cv2.arcLength(max_contour, True)
                    approx_polygon = cv2.approxPolyDP(max_contour, epsilon, True)
                    complexity = len(approx_polygon)
                else:
                    complexity = 0
                complexities.append(complexity)
            return complexities

        def calculate_weights(complexities):
            # 如果复杂度全为0，返回均匀权重
            if sum(complexities) == 0:
                return [1.0 / len(complexities)] * len(complexities)

            # 归一化复杂度，使得它们总和为1
            weights = [complexity / sum(complexities) for complexity in complexities]
        ...
        if self.contournum:
            connum_batch = [[calculate_complexities(fras_batch[i][j]) for i in range(feature_num)]
                        for j in range(batch_size)]  # [f, b]
            # connum_batch = [[np.asarray(calculate_weights(calculate_complexities(fras_batch[i][j]))) for i in range(feature_num)]
            #             for j in range(batch_size)]  # [f, b]
            batch.append(connum_batch)
        batch[0] = fras_batch
        return batch
