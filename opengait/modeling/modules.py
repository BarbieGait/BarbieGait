...
class BasicConv2d_PC(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, **kwargs):
        super(BasicConv2d_PC, self).__init__()
        self.conv = PartialConv2d(in_channels, out_channels, kernel_size,
                              stride=stride, padding=padding, bias=False, **kwargs)
...
# class SeparatePLBNFCs(nn.Module):
#     def __init__(self, parts_num, in_channels, out_channels, norm=False):
#         super(SeparatePLBNFCs, self).__init__()
#         self.p = parts_num
#         self.fc_bin = nn.Parameter(
#             nn.init.xavier_uniform_(
#                 torch.zeros(parts_num, in_channels, out_channels))) # [16,512,256]
...
#             out: [n, c_out, p]
#         """
#         x_permuted = x.permute(0, 2, 1)  # 变成 [n, p, c_in] = [8, 16, 512]
#         # 对最后一个维度 (c_in) 进行 LayerNorm
#         x_norm = self.layer_norm(x_permuted)
#         # 还原原来的维度顺序
#         x = x_norm.permute(0, 2, 1)  # 变回 [n, c_in, p] = [8, 512, 16]

#         x = x.permute(2, 0, 1).contiguous() # [16,8,512]
     #         if self.norm:
#             out = x.matmul(F.normalize(self.fc_bin, dim=1))
#         else:
#             out = x.matmul(self.fc_bin)
...
class SeparatePLBNFCs(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.norm:
            ...
            out = x.matmul(self.fc_bin)
             # 对最后一个维度 (c_in) 进行 LayerNorm
        x_norm = self.layer_norm(out)
        # # 还原原来的维度顺序
        # x = x_norm.permute(0, 2, 1)  # 变回 [n, c_in, p] = [8, 512, 16]

        # x = x.permute(2, 0, 1).contiguous() # [16,8,512]

        return x_norm.permute(1, 2, 0).contiguous()

class SeparatePLBNFCs2(nn.Module):
    def __init__(self, parts_num, in_channels, out_channels, norm=False):
        super(SeparatePLBNFCs2, self).__init__()
        self.p = parts_num
        self.fc_bin = nn.Parameter(
            nn.init.xavier_uniform_(
                torch.zeros(parts_num, in_channels, out_channels))) # [16,512,256]
    ...
    def forward(self, x):
        ...
        if self.norm:
            fcbin_list = []
            part_size = 1
            parts = torch.split(self.fc_bin, part_size, dim=0)
            for i, part in enumerate(parts):
                z = F.normalize(part, dim=1)
                fcbin_list.append(z)
            fc_bin = torch.cat(fcbin_list,dim=0)
            out = x.matmul(fc_bin)
        else:
            out = x.matmul(self.fc_bin)
        return out.permute(1, 2, 0).contiguous()

class SeparatePLN2FCs(nn.Module):
    def __init__(self, parts_num, in_channels, out_channels, norm=False):
        super(SeparatePLN2FCs, self).__init__()
        # self.p = parts_num
        ...
        self.fc_bin2 = nn.Parameter(
            nn.init.xavier_uniform_(
                torch.zeros(parts_num, out_channels, out_channels))) # [16,512,256]
        self.relu  = nn.ReLU(inplace=True)
        # self.dropout = nn.Dropout(p=0.3)
        # self.leaky_relu = nn.LeakyReLU(negative_slope=0.02)
        # self.layer_norm1 = SetBlockPLN([out_channels, 1])
        # self.layer_norm2 = SetBlockPLN([out_channels, 1])
        self.bn1 = SetBlockBN(out_channels)
        self.bn2 = SetBlockBN(out_channels)
        # self.ln1 = SetBlockBN([out_channels, 16])
        # self.ln2 = SetBlockBN([out_channels, 16])
        self.norm = norm
    ...
    def forward(self, x):
        """
            ...
            out: [n, c_out, p]
        """
        # x = x.permute(2, 0, 1)  # 变成 [n, p, c_in] = [8, 16, 512]
        # # 对最后一个维度 (c_in) 进行 LayerNorm
        # x = self.layer_norm1(x)

        x = x.permute(2, 0, 1).contiguous()
        if self.norm:
            x = x.matmul(F.normalize(self.fc_bin1, dim=1))
        else:
            x = x.matmul(self.fc_bin1)
        ...
        if self.norm:
            x = x.matmul(F.normalize(self.fc_bin2, dim=1))
        else:
            x = x.matmul(self.fc_bin2)
...
class GONFC(nn.Module):
    def __init__(self, parts_num, in_channels, out_channels, norm=False, plnnum=16):
        super(GONFC, self).__init__()
        self.p = plnnum
        self.fc_bin1 = nn.Parameter(
            nn.init.xavier_uniform_(
                torch.zeros(parts_num, in_channels, out_channels))) # [16,512,256]
        ...
        # self.layer_norm0 = SetBlockBN(in_channels)
        # self.layer_norm0 = SetBlockPLN([in_channels, 1])
        self.layer_norm1 = SetBlockPLN([out_channels, parts_num//self.p])
        self.layer_norm2 = SetBlockPLN([out_channels, parts_num//self.p])
        self.layer_norm3 = SetBlockPLN([out_channels, parts_num//self.p])
        # self.layer_norm0 = nn.LayerNorm(in_channels, eps=1e-5)
        # self.layer_norm1 = nn.LayerNorm(out_channels, eps=1e-5)
        # self.layer_norm2 = nn.LayerNorm(in_channels, eps=1e-5)
        self.norm = norm
    ...
    def forward(self, x):
        ...
        x = self.layer_norm2(x)
        # shortcut = identity.matmul(F.normalize(self.fc_bin3, dim=1))
        shortcut = self.layer_norm3(identity.matmul(F.normalize(self.fc_bin3, dim=1)))
        x = x + shortcut
        # x = self.relu(x)
...
class SeparatePLN2FCs_Res_woshare(nn.Module):
    def __init__(self, parts_num, in_channels, out_channels, norm=False):
        super(SeparatePLN2FCs_Res_woshare, self).__init__()
        # self.p = parts_num
        self.fc_bin1 = nn.Parameter(
            nn.init.xavier_uniform_(
                torch.zeros(parts_num, in_channels, out_channels))) # [16,512,256]
        ...
        self.layer_norm1 = SetBlockPLN_woshare([out_channels, 1])
        self.layer_norm2 = SetBlockPLN_woshare([out_channels, 1])
        self.layer_norm3 = SetBlockPLN_woshare([out_channels, 1])
        self.norm = norm
...
class SeparatePLBNNecks(nn.Module):
    """
        Bag of Tricks and a Strong Baseline for Deep Person Re-Identification
        CVPR Workshop:  https://openaccess.thecvf.com/content_CVPRW_2019/papers/TRMTMCT/Luo_Bag_of_Tricks_and_a_Strong_Baseline_for_Deep_Person_CVPRW_2019_paper.pdf
        Github: https://github.com/michuanhaohao/reid-strong-baseline
    ...
    def __init__(self, parts_num, in_channels, class_num, norm=True, parallel_BN1d=True):
        super(SeparatePLBNNecks, self).__init__()
        self.p = parts_num
        self.class_num = class_num
        self.norm = norm
        self.fc_bin = nn.Parameter(
    ...
    def forward(self, x):
        ...
        if self.parallel_BN1d:
            ...
            x_permuted = x.permute(0, 2, 1)  # 变成 [n, p, c_in] = [8, 16, 256]
            # 对最后一个维度 (c_in) 进行 LayerNorm
            x_norm = self.layer_norm(x_permuted)
            # 还原原来的维度顺序
            x = x_norm.permute(0, 2, 1).contiguous()  # 变回 [n, c_in, p] = [8, 256, 16]
            x = x.view(n, c, p)
        else:
            x = torch.cat([bn(_x) for _x, bn in zip(
                x.split(1, 2), self.bn1d)], 2)  # [p, n, c]
...
def pconv3x3(in_planes, out_planes, stride=1, groups=1, dilation=1):
    """3x3 convolution with padding"""
    return PartialConv2d(in_planes, out_planes, kernel_size=3, stride=stride, padding=dilation, bias=False, dilation=dilation, multi_channel = True)
...
class SetLayerNorm(nn.Module):
    def __init__(self):
        super(SetLayerNorm, self).__init__()
    ...
    def forward(self, x):
        ...
        ln1 = nn.LayerNorm([*normalized_shape[-3:]]).to(x.device)

        return ln1(x)

class SetBlockBN(nn.Module):
    def __init__(self, out_channels):
        super(SetBlockBN, self).__init__()
    ...
    def forward(self, x):
        """
            ...
            Out x: [p, n, c]
        """
        x = x.permute(1, 2, 0).contiguous() # [n,c,p]
        x = self.bn(x)
        x = x.permute(2, 0, 1).contiguous() # [n,c,p]

        return x

class SetBlockLN(nn.Module):
    def __init__(self, lnsize):
        super(SetBlockLN, self).__init__()
        self.ln = nn.LayerNorm([lnsize[0],lnsize[1]], eps=1e-5)
...
# FC中的PLN
class SetBlockPLN(nn.Module):
    def __init__(self, lnsize):
        super(SetBlockPLN, self).__init__()
        self.p = lnsize[-1]
        self.ln = nn.LayerNorm([lnsize[0],lnsize[1]], eps=1e-5)
    ...
    def forward(self, x):
        ...
        features = []
        splitnum = int(self.p)
        parts = torch.split(x, splitnum, dim=-1)
        for i, part in enumerate(parts):
            z = self.ln(part)
            features.append(z)
        x = torch.cat(features, -1) # [n,c,p]
        x = x.permute(2, 0, 1).contiguous()

        return x
class GON(nn.Module):
    def __init__(self, lnsize,part_num=16):
        super(GON, self).__init__()
        self.ln = nn.LayerNorm([lnsize[0],int(lnsize[1]/part_num),lnsize[2]])
        # if bin_num is None:
        #     bin_num = [16, 8, 4, 2, 1]
    ...
    def forward(self, x):
        ...
        """
        features = []
        part_size = x.shape[-2] // self.part_num
        parts = torch.split(x, part_size, dim=-2)
        for i, part in enumerate(parts):
            z = self.ln(part)
            features.append(z)
...
class SetBlockP3DLN(nn.Module):
    def __init__(self, lnsize,part_num=16):
        super(SetBlockP3DLN, self).__init__()
        self.ln = nn.LayerNorm([lnsize[0],int(lnsize[1]/part_num),lnsize[2]])
        self.part_num = part_num
...
class SetBlockPGBN(nn.Module):
    def __init__(self, lnsize,part_num=16):
        super(SetBlockPGBN, self).__init__()
        # self.ln = nn.LayerNorm([lnsize[0],int(lnsize[1]/part_num),lnsize[2]])
        self.gn = nn.GroupNorm(num_groups=32, num_channels=lnsize[0])
        # if bin_num is None:
        #     bin_num = [16, 8, 4, 2, 1]
        self.part_num = part_num
...
class SetBlockAPLBN(nn.Module):
    def __init__(self, lnsize, part_num=16, epsilon=1e-5, alpha=0.5):
        super(SetBlockAPLBN, self).__init__()
        self.lnsize = lnsize
        self.part_num = part_num
        # self.epsilon = epsilon
        self.alpha = nn.Parameter(torch.tensor(alpha))  # Learnable parameter for adaptive combination
        # self.gamma = nn.Parameter(torch.ones(lnsize))   # Learnable scaling factor
        # self.beta = nn.Parameter(torch.zeros(lnsize))   # Learnable shift factor
        self.ln = nn.LayerNorm([lnsize[0], int(lnsize[1] // part_num), lnsize[2]])
        self.bn = nn.BatchNorm2d(lnsize[0])
    ...
    def forward(self, x):
        """
            ...
            Out x: [n, c_out, s, h_out, w_out]
        """
        # Split input into parts based on the given part_num
        features = []
        part_size = x.shape[-2] // self.part_num
        parts = torch.split(x, part_size, dim=-2)
        for i, part in enumerate(parts):
            # # Layer Norm statistics
            # mean_ln = part.mean(dim=[1, 2, 3], keepdim=True).repeat(1,self.lnsize[1],1,1)
            # var_ln = part.var(dim=[1, 2, 3], keepdim=True, unbiased=False).repeat(1,self.lnsize[1],1,1)

            # # Batch Norm statistics
            # mean_bn = self.bn.running_mean.view(1, -1, 1, 1)
            # var_bn = self.bn.running_var.view(1, -1, 1, 1)

            # # Adaptive fusion of means and variances
            # mean_adaptive = self.alpha * mean_bn + (1 - self.alpha) * mean_ln
            # var_adaptive = self.alpha * var_bn + (1 - self.alpha) * var_ln

            # # Normalize the part using adaptive mean and variance
            # part_normalized = (part - mean_adaptive) / torch.sqrt(var_adaptive + self.epsilon)
                     # # Apply learned scale (gamma) and shift (beta)
            # z = self.gamma * part_normalized + self.beta

            z = self.alpha * self.ln(part) + (1 - self.alpha) * self.bn(part)
            features.append(z)

        # Concatenate along the part dimension
        return torch.cat(features, dim=-2)

class SetBlockAPGBN(nn.Module):
    def __init__(self, lnsize, part_num=16, epsilon=1e-5, alpha=0.5):
        super(SetBlockAPGBN, self).__init__()
        self.lnsize = lnsize
        self.part_num = part_num
        # self.epsilon = epsilon
        self.alpha = nn.Parameter(torch.tensor(alpha))  # Learnable parameter for adaptive combination
    ...
    def forward(self, x):
        ...
        parts = torch.split(x, part_size, dim=-2)
        for i, part in enumerate(parts):
            z = self.alpha * self.gn(part) + (1 - self.alpha) * self.bn(part)
            features.append(z)

        # Concatenate along the part dimension
        return torch.cat(features, dim=-2)

class SetBlockPLN_woshare(nn.Module):
    def __init__(self, lnsize, part_num=16):
        super(SetBlockPLN_woshare, self).__init__()
        self.ln_layers = nn.ModuleList([nn.LayerNorm([lnsize[0],lnsize[1]], eps=1e-5) for _ in range(part_num)])
...
class SetBlockPLBN_woshare(nn.Module):
    def __init__(self, lnsize,part_num=16):
        super(SetBlockPLBN_woshare, self).__init__()
        self.part_num = part_num
        self.ln_layers = nn.ModuleList([nn.LayerNorm([lnsize[0],int(lnsize[1]/part_num),lnsize[2]]) for _ in range(part_num)])
    ...
    def forward(self, x):
        ...
        for i, part in enumerate(parts):
            z = self.ln_layers[i](part)  # [n, h, c]
            features.append(z)  # [n, h, 1, c]
...
class BasicBlock2D_GON(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, lnsize=None, part_num=16):
        super(BasicBlock2D_GON, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
            # norm_layer_bn = nn.BatchNorm2d
            # norminplanes = int(planes/2)
        if groups != 1 or base_width != 64:
            raise ValueError(
                'BasicBlock only supports groups=1 and base_width=64')
        if dilation > 1:
        ...
        self.downsample = downsample
        self.stride = stride
        self.gon = GON(lnsize, part_num=part_num)
    ...
    def forward(self, x):
        ...
        # outin = self.partln(out)
        # outbn = self.bn1(out[:, int(out.shape[1]/2):, :, :])
        # out = torch.cat((outin, outbn),dim=1)
        out = self.conv2(out)
        out = self.bn2(out)
...
class BasicBlock2D_PLBN_woshare(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, lnsize=None, part_num=16):
        super(BasicBlock2D_PLBN_woshare, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
            # norm_layer_bn = nn.BatchNorm2d
            # norminplanes = int(planes/2)
        ...
        self.downsample = downsample
        self.stride = stride
        self.partln = SetBlockPLBN_woshare(lnsize, part_num=part_num)
...
class BasicBlock2D_PLBNab(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, lnsize=None, part_num=16):
        super(BasicBlock2D_PLBNab, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        ...
        self.downsample = downsample
        self.stride = stride
        self.partln1 = GON([int(lnsize[0]/2),lnsize[1],lnsize[2]], part_num=part_num)
        self.partln2 = GON([int(lnsize[0]/2),lnsize[1],lnsize[2]], part_num=part_num)
        self.partln  = GON(lnsize, part_num=part_num)

    def forward(self, x):
        identity = x

        out = self.conv1(x)
        out1 = self.partln1(out[:,:int(out.shape[1]/2),:,:])
        out2 = self.bn1(out[:,int(out.shape[1]/2):,:,:])
        out = torch.cat((out1,out2),dim=1)
        out = self.relu(out)

        out = self.conv2(out)
        out1 = self.partln2(out[:,:int(out.shape[1]/2),:,:])
        out2 = self.bn2(out[:,int(out.shape[1]/2):,:,:])
        out = torch.cat((out1,out2),dim=1)
...
class BasicBlock2D_PLN(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, lnsize=None, part_num=16):
        super(BasicBlock2D_PLN, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        ...
        # Both self.conv1 and self.downsample layers downsample the input when stride != 1
        self.conv1 = conv3x3(inplanes, planes, stride)
        self.partln1 = GON(lnsize, part_num=part_num)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes)
        self.partln2 = GON(lnsize, part_num=part_num)
        self.downsample = downsample
        self.stride = stride
        self.partln = GON(lnsize, part_num=part_num)
    ...
    def forward(self, x):
        ...
        out = self.conv1(x)
        out = self.partln1(out)
        out = self.relu(out)
        out = self.conv2(out)
        out = self.partln2(out)
...
class BasicBlock2D_PGBN(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, lnsize=None, part_num=16):
        super(BasicBlock2D_PGBN, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
            # norm_layer_bn = nn.BatchNorm2d
            # norminplanes = int(planes/2)
        ...
        self.partln = SetBlockPGBN(lnsize, part_num=part_num)
...
class BasicBlock2D_APLBN(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, lnsize=None, part_num=16):
        super(BasicBlock2D_APLBN, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
            # norm_layer_bn = nn.BatchNorm2d
            # norminplanes = int(planes/2)
        ...
        self.partln = SetBlockAPLBN(lnsize, part_num=part_num)
...
class BasicBlock2D_APGBN(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, lnsize=None, part_num=16):
        super(BasicBlock2D_APGBN, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
            # norm_layer_bn = nn.BatchNorm2d
            # norminplanes = int(planes/2)
        ...
        self.partln = SetBlockAPGBN(lnsize, part_num=part_num)
...
class BasicBlockP3D_GON(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1,  downsample=None, groups=1,
                 base_width=64, dilation=1, lnsize=None, norm_layer=None, part_num=16):
        super(BasicBlockP3D_GON, self).__init__()
        if norm_layer is None:
            norm_layer2d = nn.BatchNorm2d
            norm_layer3d = nn.BatchNorm3d
            # norminplanes = int(planes/2)
        ...
        self.downsample = downsample
        self.gon = SetBlockWrapper(GON(lnsize, part_num=part_num))
...
class BasicBlock3D_GON(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=[1, 1, 1],  downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, lnsize=None, part_num=1):
        super(BasicBlock3D_GON, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm3d
        if groups != 1 or base_width != 64:
            raise ValueError(
...
class BasicBlockP3D_PLBN2(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1,  downsample=None, groups=1,
                 base_width=64, dilation=1, lnsize=None, norm_layer=None, part_num=16):
        super(BasicBlockP3D_PLBN2, self).__init__()
        if norm_layer is None:
            norm_layer2d = nn.BatchNorm2d
            norm_layer3d = nn.BatchNorm3d
        ...
        self.shortcut3d = nn.Conv3d(planes, planes, (3, 1, 1), (1, 1, 1), (1, 0, 0), bias=False)
        # self.sbn        = norm_layer3d(planes)
        self.sbn        = SetBlockWrapper(SetBlockP3DLN(lnsize,part_num=part_num))

        self.downsample = downsample
        self.partln = SetBlockWrapper(GON(lnsize,part_num=part_num))
