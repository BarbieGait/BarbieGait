...
class CausalityLoss(BaseLoss):
    def __init__(self, loss_term_weight=1.0):
        super(CausalityLoss, self).__init__(loss_term_weight)

    @gather_and_scale_wrapper
    def forward(self, out2, outp2, out3, outp3, out4, outp4):
        # embeddings: [n, c, p], label: [n]
        # embeddings = embeddings.permute(
        #     2, 0, 1).contiguous().float()  # [n, c, p] -> [p, n, c]
        out2 = out2.squeeze(-1).permute(2, 0, 1).contiguous().float()  # [n, c, p] -> [p, n, c]
