...
from ..modules import HorizontalPoolingPyramid, PackSequenceWrapper, SeparateFCs, SeparateBNNecks, SetBlockWrapper, conv3x3, conv1x1, BasicBlock2D, BasicBlockP3D, BasicBlock3D
...
class FinesilGaitPP(BaseModel):
    ...
    def build_network(self, model_cfg):
        ...
        self.sil_layer1 = SetBlockWrapper(self.make_layer(BasicBlock2D, 32 * C, stride=[1, 1], blocks_num=B[0], mode=mode))
        self.map_layer1 = copy.deepcopy(self.sil_layer1)
        self.fusion = AttentionFusion(32 * C)

        self.layer2 = self.make_layer(block, 64 * C, stride=[2, 2], blocks_num=B[1], mode=mode)
        self.layer3 = self.make_layer(block, 128 * C, stride=[2, 2], blocks_num=B[2], mode=mode)
        self.layer4 = self.make_layer(block, 256 * C, stride=[1, 1], blocks_num=B[3], mode=mode)
    ...
    def forward(self, inputs):
        ...
        gtsils = data[:, :1, ...]
        predsils = data[:, -1, ...].unsqueeze(1)

        del ipts
        gtsil0 = self.map_layer0(gtsils)
        gtsil1 = self.map_layer1(gtsil0)

        predsil0 = self.sil_layer0(predsils)
        predsil1 = self.sil_layer1(predsil0)

        out1 = self.fusion(gtsil1, predsil1)
