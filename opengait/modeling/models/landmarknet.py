...
class LandmarkGait_Silh_to_Landmark(BaseModel):
    ...
    def forward(self, inputs):
        ...
        des_sils, des_pred_sils, des_hmap_sils, _, _, _ = self.Backbone(sils)

        # # landmarks_visual
        # des_hmap_sils_max_c = torch.max(des_hmap_sils, dim=1)[0].unsqueeze(2)
        des_hmap_sils_max_c = des_hmap_sils.sum(1).unsqueeze(1)
             # n, _, s, h, w = sils.size()
        ...
        retval = {
            ...
            'visual_summary': {
                ...
                'image/heatmap': des_hmap_sils_max_c.view(n*s, 1, h_kp, w_kp),
            },
            'inference_feat': {
                'sils': des_sils,
