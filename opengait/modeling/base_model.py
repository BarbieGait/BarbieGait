...
class BaseModel(MetaModel, nn.Module):
    ...
    def _load_ckpt(self, save_name):
        if "random_init" in save_name:
            self.msg_mgr.log_info("Randomly initialize all parameters (no checkpoint loaded)")
            ...
            return  # 不加载任何 checkpoint，直接 return
    ...
    def inputs_pretreament(self, inputs):
        ...
        """
        if 'contournum' in self.engine_cfg['sampler']:
            seqs_batch, labs_batch, typs_batch, vies_batch, seqL_batch, contournum_batch = inputs
            contournum = list2var(contournum_batch).float()
        else:
        ...
        if 'contournum' in self.engine_cfg['sampler']:
            return ipts, labs, typs, vies, seqL, contournum
        else:
    ...
    def run_train(model):
        ...
        for inputs in model.train_loader:
            if 'finetune' in model.cfgs['trainer_cfg']:
                training_layers = model.cfgs['trainer_cfg']['finetune'][0]['training_layers']
                for name, param in model.named_parameters():
                    if any(layer in name for layer in training_layers):
            ...
            with autocast(enabled=model.engine_cfg['enable_float16']):
                ...
                # # 关闭自动梯度跟踪与混合精度
                # with torch.no_grad():
                #     with torch.cuda.amp.autocast(enabled=False):  # 确保关闭 autocast
                #         model.eval()
                #         flops = FlopCountAnalysis(model, (ipts_detached,))
    ...
    def run_test(model):
        ...
        if rank == 0:
            ...
            if model.cfgs['data_cfg']['test_dataset_name'] == 'BarbieGait':
                p_types_list = loader.dataset.p_types_list
                g_types_list = loader.dataset.g_types_list

                info_dict.update({
                    'labels': label_list, 'types': types_list, 'views': views_list, 'p_types': p_types_list, 'g_types': g_types_list,})
            else:
                info_dict.update({
